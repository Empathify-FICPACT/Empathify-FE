...
export default function Interaktif() {
  ...
  const handleStartSession = async () => {
    ...
    try {
      ...
      if (data.meta?.success && data.data) {
        ...
        const sortedScenarios = data.data.scenarios.sort(
        ...
        showErrorMessage("Gagal memulai sesi simulasi cerita.");
      }
    } catch (error) {
      console.error(error);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await apiFetch(
        `/api/v1/story/sessions/${sessionId}/answers`,
        {
          method: "POST",
          body: JSON.stringify({
      ...
      console.error(error);
      showErrorMessage("Terjadi kesalahan koneksi saat mengirim jawaban.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleNext = async () => {
    ...
    if (currentIndex + 1 < scenarios.length) {
      ...
      try {
        const res = await apiFetch(
          `/api/v1/story/sessions/${sessionId}/complete`,
          {
            method: "PATCH",
          },
        );
          ...
          <div className="z-10 flex flex-col items-center max-w-xl mx-auto -mt-10 md:-mt-20 w-full">
            ...
            <div className="text-center px-4 mb-8">
              ...
              <p className="text-xs sm:text-sm md:text-base text-gray-400 font-medium mb-6">
                Baca cerita dan pilih tindakan yang tepat bersosial dengan
                benar.
              </p>
          ...
          <div className="w-full max-w-5xl mx-auto z-10 flex flex-col mt-4 md:mt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5 w-full">
              {optionsMap.map((opt) => {
                ...
                let buttonClass =
                  "bg-white border-gray-200 text-gray-700 hover:border-[#2cb46c] hover:bg-[#eafff2]";
                let circleClass = "bg-[#eafff2] text-[#2cb46c]";

                if (isSubmitted) {
                  if (isCorrect) {
                    buttonClass =
                      "bg-[#2cb46c] border-[#2cb46c] text-white shadow-md transform scale-[1.02] z-10";
                ...
                return (
                  ...
                  >
                    <div
                      className={`shrink-0 uppercase flex items-center justify-center w-10 h-10 md:w-12 md:h-12 rounded-full font-bold text-base md:text-lg mr-4 md:mr-6 transition-colors shadow-sm ${circleClass}`}
                    >
                      {opt.id}
                    </div>
            ...
            <div className="flex items-center justify-end mt-10 md:mt-12 w-full mb-10">
              {!isSubmitted && !isReviewMode ? (
                ...
                >
                  {isLoading
                    ? "Memuat..."
                    : isReviewMode && currentIndex === scenarios.length - 1
                      ? "Selesai Review"
                      : "Selanjutnya"}
                </button>
              )}
        ...
        <div className="z-10 flex flex-col items-center max-w-5xl mx-auto w-full mt-2 md:mt-4 lg:mt-6">
          <div className="flex flex-col items-center w-full max-w-2xl mx-auto mb-8 md:mb-12">
            ...
            <p className="text-base sm:text-lg md:text-xl text-gray-500 font-medium mb-8 md:mb-12 text-center max-w-xl">
              Pingo suka caramu mengambil keputusan. Kamu makin peduli pada
              sekitar!
            </p>
